	...
	def create
		...
		if params[:courserating][:rating]
			...
			if @findrate
				...
				current_user.courseratings.create!(course_id: @course.id, rating: @rating, review_title: "+!", review_content: "+!")
			end
		...
		if params[:courserating][:review_title]
			...
			if @findrate
				...
				current_user.courseratings.create!(course_id: @course.id, review_title: @review_title, review_content: @review_content, rating: 0)
			end
			redirect_to course_path(@course)
		end
	end
